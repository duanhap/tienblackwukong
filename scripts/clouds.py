...
import pygame
class Cloud:
    def __init__(self,pos,img,speed,depth,scale=1.2):
...
class Clouds:
    def __init__(self,cloud_images,count=16,scale=4.5):
        ...
        for i in range(count):
            self.clouds.append(Cloud((random.random()*99999,random.random()*99999),random.choice(cloud_images),random.random()*0.05 +0.5,random.random()*0.6+0.2,scale))
